...
low_light_folder= '/MATLAB Drive/NLIEE/LoLI_inp/LoLI_inp'

low_light_files = dir(fullfile(low_light_folder, '*.jpg'));  % Adjust the pattern if needed

% Filter only directories that do not end with ".zip"
%enhanced_folders = all_items([all_items.isdir] & ~endsWith({all_items.name}, '.zip'));

enhanced_folders= dir(fullfile('/MATLAB Drive/NLIEE/LoLI_enhanced/LoLI_enhanced', '*.jpg'));
...
for j=1: length(enhanced_folders)
    enhanced_path= fullfile('/MATLAB Drive/NLIEE/LoLI_enhanced/LoLI_enhanced', enhanced_folders(j).name)
    low_light_path= fullfile(low_light_folder, enhanced_folders(j).name)
...
% 
% 
dataTable = table(filename', Quality_Scores',img_size_ls', ...
                  'VariableNames', {'Filename', 'Quality_Scores', 'img_size_ls'});


% Specify the CSV file name
csvFileName = '/MATLAB Drive/NLIEE/LOLI_output.csv';
