...
low_light_folder= '/MATLAB Drive/NLIEE/CLED_inputs/CLED_inputs'
...
enhanced_folders= dir(fullfile('/MATLAB Drive/NLIEE/CLED_outputs/CLED_outputs', '*.png'));
...
for j=1: length(enhanced_folders)
    enhanced_path= fullfile('/MATLAB Drive/NLIEE/CLED_outputs/CLED_outputs', enhanced_folders(j).name)


    % Apply regular expression
    pattern = '^(.*?)(\d+)_([\d]+)_level(\d+)';
    tokens = regexp(enhanced_folders(j).name, pattern, 'tokens');
    
    % Extract the values
    prefix = tokens{1}{1};  % The part before the numbers (ch2_ohr_ncp_20200229T0938004033_b_brw_d32)
    value1 = str2double(tokens{1}{2});  % First number (9)
    value2 = str2double(tokens{1}{3});  % Second number (2)
    level_number = str2double(tokens{1}{4});  % Number following level (6)

    Levels(idx)=level_number

    low_light_path= fullfile(low_light_folder, [prefix,tokens{1}{2},'_',tokens{1}{3},'.png',])
    img_size_ls{idx}=size(imread(low_light_path));
    quality_score= LOE_FN(low_light_path, enhanced_path)

    filename{idx}=enhanced_folders(j).name;
...
% 
% 
dataTable = table(filename', Quality_Scores', img_size_ls', Levels',...
                  'VariableNames', {'Filename', 'Quality_Scores','img_size_ls','Levels'});

% Specify the CSV file name
csvFileName = '/MATLAB Drive/NLIEE/CLED_LOE.csv';
